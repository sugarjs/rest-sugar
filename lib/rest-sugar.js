...
function initAPI(app, name, model, queries, auth, success) {
    var groupOps = {
        post: function(req, res) {
            queries.create(model, getParams(req), operate(res, success));
        },
        get: function(req, res) {
            ...
            else queries.getAll(model, query, operate(res, success));
        },
        'delete': function(req, res) {
            queries['delete'](model, req.body._id, operate(res, success));
        }
    };
    ...
    app.get(name + '/count', handle(auth, function(req, res) {
        queries.count(model, operate(res, success));
    }));
    ...
    var idOps = {
        get: function(req, res) {
            ...
            else queries.get(model, req.params.id, req.query.fields, operate(res, success));
        },
        put: function(req, res) {
            queries.update(model, req.params.id, getParams(req), operate(res, success));
        },
        'delete': function(req, res) {
            queries['delete'](model, req.params.id, operate(res, success));
        }
    };
