...
function key(name, value, check) {
    ...
    return function(fn) {
        return function(req, res) {
            if(!check(req)) return unauthorized(res);
...
function initAPI(app, name, model, queries, auth) {
    var groupOps = {
        post: function(req, res) {
            queries.create(model, getParams(req), ret(res), ret(res));
        },
        get: function(req, res) {
            var method = req.query.method;

            if(method) getOp(groupOps, method)(req, res);
    ...
    var idOps = {
        get: function(req, res) {
            ...
            if(method) getOp(idOps, method)(req, res);
            else queries.get(model, req.params.id, req.query.fields, ret(res), err(res));
        },
        put: function(req, res) {
            queries.update(model, req.params.id, getParams(req), ret(res), err(res));
        },
